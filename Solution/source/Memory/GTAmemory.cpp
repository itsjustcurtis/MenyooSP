...
typedef CVehicleModelInfo*(*InitVehicleArchetypeLegacy_t)(const char*, bool, unsigned int);
typedef __int64(*InitVehicleArchetypeEnhanced_t)(uint32_t, const char*);
bool isEnhanced = (GTAmemory::GetGameVersion() >= eGameVersion::VER_1_0_812_8);

GetModelInfo_t GetModelInfo;

std::unordered_map<unsigned int, std::string> g_vehicleHashes;
CallHook<InitVehicleArchetypeLegacy_t> * g_InitVehicleArchetypeLegacy = nullptr;
CallHook<InitVehicleArchetypeEnhanced_t>* g_InitVehicleArchetypeEnhanced = nullptr;
CVehicleModelInfo* initVehicleArchetypeLegacy_stub(const char* name, bool a2, unsigned int a3) {
	g_vehicleHashes.insert({ GET_HASH_KEY(name), boost::to_lower_copy(name) });
	return g_InitVehicleArchetypeLegacy->fn(name, a2, a3);
}
__int64 initVehicleArchetypeEnhanced_stub(uint32_t a1, const char* name) {
	g_vehicleHashes.insert({ GET_HASH_KEY(name), boost::to_lower_copy(name) });
	...
	const char* pattern = isEnhanced ? "\xE8\x00\x00\x00\x00\x43\x89\x44\x2C" : "\xE8\x00\x00\x00\x00\x48\x8B\x4D\xE0\x48\x8B\x11";
	const char* mask = isEnhanced ? "x????xxxx" : "x????xxxxxxx";
	auto addr = GTAmemory::FindPattern(pattern, mask);
	if (!addr) {
	...
	if (isEnhanced) {
		...
		g_InitVehicleArchetypeLegacy = HookManager::SetCall(addr, initVehicleArchetypeLegacy_stub);
	}
}
